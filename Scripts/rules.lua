					...
					for i=1,variations do
						...
						for wordid=1,#sentences do
							if (variantcount[wordid] > 0) then
								...
								if (tiletype ~= 5) then
									if (stage == 0) then
										if (tiletype == 0) and (prevtiletype ~= 9) then
											prevstage = stage
											stage = 2
									...
									elseif (stage == 2) then
										if (wordid ~= #sentences) then
											...
											elseif ((tiletype == 7) and (stage2reached == false)) then
												...
												prevstage = stage
												stage = 3
											elseif (tiletype == 6) and (prevtiletype ~= 4) and (prevtiletype ~= 9) then
												prevstage = stage
												stage = 4
											elseif (tiletype ~= 4) and (tiletype ~= 9) then
												prevstage = stage
												stage = -1
												stop = true
											end
									...
									elseif (stage == 3) then
										...
										if ((tiletype == 0) and (prevtiletype ~= 9)) or (tiletype == 2) or (tiletype == 8) then
											prevstage = stage
											stage = 5
										elseif (tiletype ~= 4) and (tiletype ~= 9) then
											stage = -1
									...
									elseif (stage == 4) then
										if (wordid < #sentences) then
											if ((tiletype == 0) and (prevtiletype ~= 9)) or ((tiletype == 2) and stage3reached) or ((tiletype == 8) and stage3reached) then
												prevstage = stage
												stage = 2
									...
									elseif (stage == 5) then
										if (wordid ~= #sentences) then
											if (tiletype == 1) and doingcond and (prevtiletype ~= 4) and (prevtiletype ~= 9) then
												stage2reached = true
												doingcond = false
												prevstage = stage
												stage = 3
								...
								if (tiletype == 9) then
									if (reversestatus == 0) then
										reversestatus = tileid
									end
								else
									if (stop == false) and (tiletype ~= 0) then
										reversestatus = 0
								...
								if (stop == false) then
									...
									if (tiletype == 0) and (prevtiletype ~= 9) and (prevtiletype == 0) and (notstatus ~= 0) then
										notstatus = 0
									end
						...
						if valid then
							for index,wdata in ipairs(sentence) do
								...
								if (wtype == 1) or (wtype == 3) or (wtype == 7) then
									wcategory = 1
								elseif (wtype ~= 4) and (wtype ~= 6) and (wtype ~= 9) then
									wcategory = 0
								else
									table.insert(extraids_ifvalid, {prefix .. revprefix .. wname, wtype, wid})
								...
								if (wcategory == 0) then
									...
									if allowed then
										table.insert(group, {prefix .. revprefix .. wname, wtype, wid})
									else
									...
									if (index < #sentence) then
										...
										if (realname == nil) then
											...
											if (wtype == 1) then
												...
												stage = 1
												local target = {prefix .. revprefix .. wname, wtype, wid}
											...
											elseif (wtype == 3) then
												allowedwords = {0}
												local cond = {prefix .. revprefix .. wname, wtype, wid}
								...
								--makes "reverse reverse" do nothing, like "not not"
								
								if (wtype == 9) then
									if (revprefix == "reverse ") then
										revprefix = ""
									else
										revprefix = "reverse "
									end
								...
								if (wname ~= extraids_current) and (string.len(extraids_current) > 0) and (wtype ~= 4) then
									for a,extraids_valid in ipairs(extraids_ifvalid) do
										table.insert(extraids, {prefix .. revprefix .. extraids_valid[1], extraids_valid[2], extraids_valid[3]})
									end
...
function addoption(option,conds_,ids,visible,notrule)
	...
	if (#option == 3) then
		...
		--checks table and inserts appropriate value if word has "reverse" before it
		local hasreverse = false
		local revrules = {target,verb,effect}
		for i,v in ipairs(revrules) do
			local name = v
			local isnot = false
			local isrev = false
			...
			if string.sub(name, 1, 8) == "reverse " then
				isrev = true
				name = string.sub(name, 9)
			end
			if isrev and reversefeatures[name] then
				hasreverse = true
				local nameprefix = ""
				if isnot then
					nameprefix = "not "
				end
				revrules[i] = nameprefix .. reversefeatures[name]
			end
		end
		...
		if hasreverse then
			...
			addoption(revrules,newconds,ids,false,nil)
		end
		
		--code that makes "baba reverse is keke" work
		if (verb == "reverse is") then
			addoption({effect,"is",target},newconds,ids,false)
		end
		
		--it also works with has!
		if (verb == "reverse has") then
			addoption({effect,"has",target},newconds,ids,false)
		end
		
		--and the modded copy!!
		if (verb == "reverse copy") then
			addoption({effect,"copy",target},newconds,ids,false)
		end
					...
					for a,d in ipairs(idlist) do
						for c,b in ipairs(d) do
							if (b ~= 0) then
								...
								if (ruleids[b] == nil) and (#undobuffer > 1) and (alreadyrun == false) then
...
function findword(text,nexts,tilename)
	...
	if (string.len(text) > 0) then
		for i,v in ipairs(customobjects) do
			if (string.len(text) > string.len(tilename) + 1) and (string.sub(v, 1, 2) == string.sub(text, -2)) and (v ~= text) then
				secondaryfound = true
			end
			
			if (string.len(text) > string.len(tilename) + 1) and (string.sub(v, 1, 7) == "text_" .. string.sub(text, -2)) and (v ~= alttext) then
				secondaryfound = true
			end
			
			if (string.len(v) >= string.len(text)) and (string.sub(v, 1, string.len(text)) == text) then
				found = true
				custom = true
				table.insert(customname, v)
			end
			
			if (string.len(v) >= string.len(alttext)) and (string.sub(v, 1, string.len(alttext)) == alttext) then
				found = true
				custom = true
				table.insert(customname, v)
			end
	...
	elseif custom then
		for i,v in ipairs(customname) do
			if (text == v) or (alttext == v) then
				wtype = 8
				name = v
				break
			end
