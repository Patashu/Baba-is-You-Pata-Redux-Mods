...
function writerules(parent,name,x_,y_)
	...
	if (texthide == nil) then
		for i,rules in ipairs(visualfeatures) do
			...
			y = basey + (((i - 1) % linelimit) + 1) * f_tilesize * 0.8
...
function create(name,x,y,dir,oldx_,oldy_,float_,skipundo_)
...
function destroylevel(special_)
	...
	if (special == "infinity") then
		if (HACK_INFINITY >= 200) then
			writetext("infinite loop",0,screenw * 0.5 - 12,screenh * 0.5 + 60,0,true,2,true,{4,1},3)
			HACK_INFINITY = 0
